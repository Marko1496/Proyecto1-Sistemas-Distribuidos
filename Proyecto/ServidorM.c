                        ...
                        if (FD_ISSET( sd, &readfds))
                        {
                                ...
                                else
                                {
                                        ...
                                        if(accion == 1) {
                                                ...
                                                if(strcmp(paginas[numero_pagina].ip, "VACIO") == 0) {
                                                        strcpy(mensajeS1,"1");
                                                        strcat(mensajeS1, "-");
                                                        sprintf(numPagS1,"%i", numero_pagina);
                                                        strcat(mensajeS1, numPagS1);
                                                        strcat(mensajeS1, "-");
                                                        paginas[numero_pagina].socket = sd;
                                                        strcpy(paginas[numero_pagina].ip,inet_ntoa(address.sin_addr));
                                                        strcat(mensajeS1, inet_ntoa(address.sin_addr));
                                                        strcat(mensajeS1, "-");
                                                        sprintf(socketS,"%i", sd);
                                                        strcat(mensajeS1, socketS);
                                                } else {
                                                        strcpy(mensajeS1,"0");
                                                        strcat(mensajeS1, "-");
                                                        sprintf(numPagS1,"%i", numero_pagina);
                                                        strcat(mensajeS1, numPagS1);
                                                        strcat(mensajeS1, "-");
                                                        strcat(mensajeS1, paginas[numero_pagina].ip);
                                                        strcat(mensajeS1, "-");
                                                        sprintf(socketS,"%i", paginas[numero_pagina].socket);
                                                        strcat(mensajeS1, socketS);
                                                }
                                                strcat(mensajeS1, "-");
                                                sprintf(versionS,"%i", paginas[numero_pagina].version);
                                                strcat(mensajeS1, versionS);


                                                imprimirPaginas(paginas, cantidadPag);
                                                send(sd, mensajeS1, strlen(mensajeS1), 0 );
                                        }
                                        else if(accion == 2) {
                                                system("clear");
                                                printf("Ip guardada: %s\n", inet_ntoa(address.sin_addr));
                                                ...
                                                strcat(mensajeS1, "-");
                                                paginas[numero_pagina].version = paginas[numero_pagina].version+1;
                                                sprintf(versionS,"%i", paginas[numero_pagina].version);
                                                strcat(mensajeS1, versionS);
